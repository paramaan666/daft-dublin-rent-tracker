...
<body>
  <header>
    ...
    <div class=\"toolbar\">
      <input id=\"search\" placeholder=\"Hledat lokalitu / název…\" />
      <select id=\"status\">
  ...
  <script>
    ...
    function render() {
      ...
      document.getElementById('meta').textContent = `${visibleActive} viditelných aktivních / ${active} aktivních / ${listings.length} celkem. Skryté: ${hidden}. Aktualizováno: ${updatedAt}.`;
      let rows = listings.filter(x => {
        ...
        if (q) {
          const hay = `${x.title || ''} ${x.location || ''} ${x.postcode || ''}`.toLowerCase();
    ...
    function isHidden(x) {
      return hiddenIds.has(listingKey(x));
    }
    ...
    function card(x) {
      const price = x.price_eur ? euro.format(x.price_eur) + ' / měsíc' : 'Cena neznámá';
      const reasons = (x.review_reasons || []).join(', ');
      const id = listingKey(x);
      const idArg = esc(JSON.stringify(id));
      const hiddenListing = isHidden(x);
      const badges = [
        `<span class=\"badge ${x.status === 'active' ? '' : 'inactive'}\">${esc(x.status || 'active')}</span>`,
        ...
        x.double_beds ? `<span class=\"badge\">ověřený double: ${esc(x.double_beds)}</span>` : ''
      ].join('');
